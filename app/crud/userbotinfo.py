...
async def get_userbotinfo(db: Session, userbotinfo_id: int):
    return db.query(UserbotInfo).filter(UserbotInfo.id == userbotinfo_id).first()


async def get_userbotinfo_by_phone(db: Session, phone: str):
    return db.query(UserbotInfo).filter(UserbotInfo.phone == phone).first()


async def get_userbotinfos(db: Session, skip: int = 0, limit: int = 10):
    return db.query(UserbotInfo).offset(skip).limit(limit).all()


async def create_userbotinfo(db: Session, userbotinfo: UserbotInfo):
