...
def create_userbotinfo(userbotinfo: UserBotInfoCreate, db: Session = Depends(get_db)):
...
def read_userbotinfo(userbotinfo_id: int, db: Session = Depends(get_db)):
...
def read_userbotinfos(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
